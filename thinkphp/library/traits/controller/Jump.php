...
trait Jump
{
     ...
     * @return void
     */
    protected function showCode($code = '1001', $msg ='', $data = [],$url='', $count = '')
    {
        $type = $this->getResponseType();
        if (is_null($url)) {
            $url = $this->app['request']->isAjax() ? '' : 'javascript:history.back(-1);';
        ...
        $msg = $msg?$msg:(isset(ReturnCode::$return_code[$code])?ReturnCode::$return_code[$code]:'未定义消息');
        $result = [
            'code' => $code,
            'msg'  => $msg,
            'data' => $data,
        ...
        $response = Response::create($result, $type)->options(['jump_template' => $this->app['config']->get('dispatch_code_tmpl')]);
